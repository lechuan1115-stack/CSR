...
#         group="p4",
#         device=device,
#         num_dynamic_kernels=4,  # 假设我们想使用5个动态卷积核
#     ):
#         super().__init__()
...
#
#         # 添加动态卷积层
#         # self.dynamic_conv = DynamicConv2d(in_channels, out_channels, num_dynamic_kernels, filter_size, stride=stride,
#         #                                   padding=padding)
#         self.dynamic_conv = DynamicConv2d(in_channels, filter_size, stride=stride, padding=padding,
#                                           num_kernels=num_dynamic_kernels)
#
...
#         self.device = device
#
#         # Ensure stride is a single integer or a tuple of two integers
...
#         # self.kernel_generator = KernelGenerator(in_channels, out_channels, filter_size, num_kernels=4)
#
...
if __name__ == "__main__":
    ...
    print(device)
    x = torch.randn(128, 2, 1, 4096, requires_grad=True).to(device)
    g_conv = GConv2d(2, 32, filter_size=5, stride=1, padding=2, device=device)  # Reduced channels
