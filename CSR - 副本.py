...
def main():
    ...
    for SNR in range(20,22,2):
        ...
        print("#######开始测试")
        model.load_state_dict(torch.load(path1))#加载训练好的模型参数，恢复网络权重
        print('导入模型成功')
        # from torchsummary import summary
        # summary(model, (2, 4800))
        start_time = time.time()#记录开始时间
        real_label, pre_label, feature_save = test(model, testloder, use_gpu)#调用测试函数，对testloader中的测试数据进行推理，返回：真实标签，预测标签，模型中间提取的特征（通常用于后续的可视化等任务）
        elapsed = time.time() - start_time#计算测试花费的总时间
        print(len(test_set))#打印测试集中样本数量
        print("推理时间为:{:5f}ms".format(elapsed / len(test_set) * 1000))#计算平均推理时间，格式化输出
        #初始化三个列表：
        reallabel = []#最终拼接的所有真实标签
        predlabel = []#最终拼接的所有预测标签
        feature_temp = []
        for s in range(len(feature_save)):
            feature_temp.append(torch.tensor(feature_save[s]))#暂存所有测试样本的特征张量
        # a = torch.tensor(feature_save)
        # print(type(torch.tensor(feature_save)))
        # print(a.shape)
        feature = torch.cat(feature_temp, 0)#将多个batch的特征张量按照维度0拼接成一个大的二维特征张量
        for i in range(len(real_label)):
            reallabel += real_label[i]
            predlabel += pre_label[i]#将每个batch中的标签扁平化后拼接到reallabel和predlabel
        np.save(osp.join(file + args.save_dir, str(SNR) + "reallabel.npy"), reallabel)
        np.save(osp.join(file + args.save_dir, str(SNR) + "predlabel.npy"), predlabel)
        np.save(osp.join(file + args.save_dir, str(SNR) + "feature.npy"), feature)#将三个关键输出保存为NumPy .npy文件，便于后续评估分析或画图
        #绘制混淆矩阵图
        import Confusion_matrix
        Confusion_matrix.plot_confusion_matrix(reallabel, predlabel, file + args.save_dir, SNR,
                                               normalize=True, cmap=plt.cm.Blues)
        plt.close()
        # Confusion_matrix.plot_confusion_matrix(reallabel, predlabel, file + args.save_dir,
...
def test(model, testloader, use_gpu):
    """测试：返回 real_label, pre_label, feature_save，并正确统计 Top-1/2/3/5"""
    model.eval()
    ...
    with torch.no_grad():
        for data, labels in testloader:
            ...
            labels = torch.squeeze(labels).long()       # [B]

            outputs, feature = model(data)              # outputs: [B, C]
            preds = outputs.argmax(dim=1)               # [B]

            # ==== 收集结果（保持你原来的返回格式）====
            real_label.append(labels.detach().cpu().numpy().tolist())
            pre_label.append(preds.detach().cpu().numpy().tolist())
            feature_save.append(feature.detach().cpu().numpy().tolist())

            # ==== 正确计算 Top-k ====
            maxk = 5
            _, pred_topk = outputs.topk(maxk, 1, True, True)  # [B, K]
            pred_topk = pred_topk.t()                         # [K, B]
            # correct[k, b] 表示第 b 个样本是否在前 k+1 的预测里
            correct = pred_topk.eq(labels.view(1, -1).expand_as(pred_topk))  # [K, B] (bool)

            # Top-1: 只看第一行
            top1_correct += correct[:1].reshape(-1).float().sum().item()
            # Top-2/3/5: 看前 k 行里是否有任意命中
            top2_correct += correct[:2].any(dim=0).float().sum().item()
            top3_correct += correct[:3].any(dim=0).float().sum().item()
            top5_correct += correct[:5].any(dim=0).float().sum().item()
    ...
    print(f"Top1:{top1_acc*100:5.2f}%  Top2:{top2_acc*100:5.2f}%  "
          f"Top3:{top3_acc*100:5.2f}%  Top5:{top5_acc*100:5.2f}%")
